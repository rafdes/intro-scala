...
trait FunSets extends FunSetsInterface {
   ...
   */
  def intersect(s: FunSet, t: FunSet): FunSet = (a: Int) => contains(s, a) && contains(t, a)

  /**
   * Returns the difference of the two given sets,
   * the set of all elements of `s` that are not in `t`.
   */
  def diff(s: FunSet, t: FunSet): FunSet = (a: Int) => contains(s, a) && !(contains(t, a))
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = (a: Int) => contains(s, f(a))
